...
const PATH_TO_PROTO_GENERATED = path.join(PATH_TO_ROOT, 'src', 'gen');

main();

async function main() {
  await generateIndexForGeneratedTypeScript(PATH_TO_PROTO_GENERATED);
}

async function generateIndexForGeneratedTypeScript(
  dir: string
    ...
    if (ent.isDirectory()) {
      ...
      for (const subEnt of subDirents) {
        if (subEnt.isDirectory()) {
          promises.push(
            generateIndexForNamespace(path.join(thisDir, subEnt.name))
          );
        }
      }
    }
...
async function fixAutoGen(filePath: string): Promise<void> {
  ...
  code = code.replace(
    ...
    (_, $1, $2, $3) => {
      const hasExt = path.extname($3) !== '';
      const pathname = hasExt ? $3 : `${$3}.js`;
...
async function generateIndexForNamespace(dir: string): Promise<void> {
  ...
  for (const ent of dirents) {
    ...
    imports += `export * from './${ent.name.replace(
      /\.ts$/,
      ''
    )}';\n`;
